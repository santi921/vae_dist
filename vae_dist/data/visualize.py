...
def plot_nodes_edge(file="../../data/pdbs_processed/1a4e.pdb"):
    ...
    for i in range(len(atom_list)):
        G.add_node(i, xyz=xyz_list[i], atom=atom_list[i])

    for i in range(len(bond_list)):
        G.add_edge(bond_list[i][0], bond_list[i][1])
    ...
    trace_nodes = go.Scatter3d(
        ...
        marker=dict(
            symbol="circle", size=size, color=color, colorscale="Viridis", opacity=0.8
        ),
    )
...
def get_cones_viz_from_pca(
    ...
    for ind, pca_comp in enumerate(pca_obj.components_):
        ...
        u_1, v_1, w_1 = split_and_filter(
            comp_vect_field, cutoff=95, std_mean=True, min_max=False
        )
