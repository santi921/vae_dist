...
def test_model_construction(): 
    ...
    model_list = ['escnn', 'esvae', 'cnn', 'vae', "cnn_supervised", "escnn_supervised"] 
    

    for model_select in model_list:
        if model_select == 'escnn':
            options = json.load(open('../vae_dist/scripts/options/options_escnn_default.json'))
            model = construct_model("escnn", options)
            model.to(device)
        elif model_select == 'esvae':
            options = json.load(open('../vae_dist/scripts/options/options_esvae_default.json'))
            model = construct_model("esvae", options)
            model.to(device)
        elif model_select == 'cnn':
            options = json.load(open('../vae_dist/scripts/options/options_cnn_default.json'))
            model = construct_model("cnn", options)
            model.to(device)
        elif model_select == 'vae':
            options = json.load(open('../vae_dist/scripts/options/options_vae_default.json'))
            model = construct_model("vae", options)
            model.to(device)
        elif model_select == 'cnn_supervised':
            options = json.load(open('../vae_dist/scripts/options/options_cnn_default_supervised.json'))
            model = construct_model("cnn_supervised", options)
            model.to(device)
        elif model_select == 'escnn_supervised':
            options = json.load(open('../vae_dist/scripts/options/options_escnn_default_supervised.json'))
